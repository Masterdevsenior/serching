...
def recrear_tabla():
    """Eliminar y recrear la tabla con las nuevas restricciones"""
    try:
        logger.info("=== RECREANDO TABLA ===")
        eliminar_tabla()
        return crear_tabla()
    except Exception as e:
        logger.error(f"Error al recrear la tabla: {e}")
        return False
...
def guardar_datos(datos_form):
    ...
    try:
        ...
        try:
            # Validar y convertir fecha_hora con manejo de diferentes formatos
            fecha_str = datos['fecha_hora']
            ...
            if fecha_convertida is None:
                logger.error(f"No se pudo convertir la fecha: {fecha_str}")
                return False
                
            datos['fecha_hora'] = fecha_convertida
            ...
            # Validar peso_carga con manejo especial para ejercicios sin carga
            datos['peso_carga'] = float(datos['peso_carga'])
            if datos['peso_carga'] < 0:  # Permitir 0 para ejercicios sin carga
                logger.error("El peso de carga no puede ser negativo")
                return False
            ...
            if datos['num_repeticiones'] < 0:  # Permitir 0 para ejercicios sin repeticiones
                logger.error("El número de repeticiones no puede ser negativo")
                return False
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                peso_corporal, altura, longitud_brazo, longitud_pierna, longitud_torso,
                antecedentes_lesiones, limitaciones_movilidad, experiencia_levantamiento,
                sin_contraindicaciones, ajuste_traje, calibracion_sincronizada, sensores_posicion,
                tipo_ejercicio, nivel_riesgo, peso_carga, duracion_minutos, num_repeticiones,
        ...
        logger.error(f"Error inesperado al obtener registros: {e}")
        return []
    finally:
        if cursor:
            cursor.close()
